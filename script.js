...
function getInitialState() {
  return {
    ...
    breakInfinity: (typeof getDefaultBreakInfinityState === 'function') ? getDefaultBreakInfinityState() : { unlocked: false, points: 0, upgrades: {} },
    lastTick: Date.now(),
    autobuyerTimer: 0,
...
function gameLoop() {
  ...
  game.generators.forEach((gen, i) => {
    ...
    if (i === 0) {
      ...
      let produced;
      if (!isFinite(pps)) {
        // 通常のnumber演算がオーバーフローした場合。
        // Break Infinity解放後はDecimalとして扱い、上限なく成長を続けられるようにする。
        // 未解放時は従来通り1.79e308/秒でクランプする。
        produced = breakActive ? toDecimal(pps).mul(dt) : new Decimal(1.79e308 * dt);
      } else {
...
function revealNextGenerator(index) {
  ...
  if (gen && gen.amount >= 1 && next && !next.unlocked) {
    next.unlocked = true;
...
function triggerBigCrunch() {
  ...
  // Break Infinityポイント付与（解放済みの場合のみ）
  if (typeof grantBreakInfinityPointsOnCrunch === 'function') grantBreakInfinityPointsOnCrunch();
  ...
  if (data) {
    try {
      ...
      game.breakInfinity = {
        ...
        upgrades: { ...((parsed.breakInfinity && parsed.breakInfinity.upgrades) || {}) }
      };
      ...
      if (!Array.isArray(game.stats.totalMkPurchased) || game.stats.totalMkPurchased.length !== 8) {
        ...
        game.stats.totalMkPurchased = game.generators
          ? game.generators.map(g => g.amount || 0)
          : [0,0,0,0,0,0,0,0];
      }
        ...
        game.generators.forEach((gen, i) => {
          ...
          if (!hadUnlockedField) {
            if (i === 0) {
              ...
              gen.unlocked = (gen.amount > 0) || !!(prev && (prev.amount > 0 || prev.unlocked));
            }
          }
        });
