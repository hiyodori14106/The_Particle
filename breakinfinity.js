...
// =========================================================
// breakinfinity.js（アダプター部分）
//
// 上のDecimalライブラリ（break_infinity.js）をこのゲームに
// 適応させるための最小限のアダプター。
// 独自の「Break Infinityアップグレード」等は作らず、
// ライブラリ本来の役割どおり「1.78e308を超える桁数を扱える
// ようにする」ことだけに専念する。
//
// game.breakInfinity.unlocked が true になると、
// script.js側の粒子上限チェックがDecimalベースの計算に切り替わり、
// 1.78e308を超えて進行できるようになる。
//
// 依存関係:
...
// （toDecimal はscript.js側で定義されるが、この関数が実際に呼ばれる時点
// （DOMContentLoaded後）には両ファイルとも読込済みのため問題なく使える）
function biGetIP() {
  if (!game.infinity) return new Decimal(0);
  if (typeof toDecimal === 'function') {
    game.infinity.ip = toDecimal(game.infinity.ip);
  } else if (!(game.infinity.ip instanceof Decimal)) {
    game.infinity.ip = new Decimal(game.infinity.ip || 0);
  }
...
function tryUnlockBreakInfinity() {
  ...
  if (currentIP.lt(BREAK_INFINITY_UNLOCK_IP)) return;
  ...
  if (typeof showModal === 'function') {
    showModal({
      title: '上限突破',
      body: '粒子の上限が撤廃されました。<br>1.78e308を超えて進行できます。',
...
function updateBreakInfinityUnlockSection() {
  ...
  if (ipDisplay) ipDisplay.textContent = format(currentIP);
...
function updateBreakInfinityTab() {
  ...
  `;
  const ipEl2 = document.getElementById('bi-current-ip2');
  if (ipEl2) ipEl2.textContent = format(currentIP);
}
