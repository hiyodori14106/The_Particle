// =========================================================
// break_infinity.js ライブラリ本体（Patashu氏作）
// 巨大数(Decimal)を扱うためのクラス。ここに同梱することで
// breakinfinity.js 1ファイルでBreak Infinity機能が完結する。
// =========================================================
...
// =========================================================
// breakinfinity.js
// Break Infinity システム（Antimatter Dimensionsにおける
// 「Break Infinity」と同じ考え方の、Infinityのさらに上位に
// 位置するシステム）。
//
// Break Infinity に関する状態・アップグレード・通貨・関数は
// すべてこの中にまとめて管理する（実績・チャレンジと同じく、
// 配列/オブジェクトでデータを管理し、今後の追加に対応しやすい
// 構造にしている）。
//
// このファイル1つでBreak Infinity関連の処理をすべて完結させる
// （break_infinity.js の Decimal ライブラリ本体もこのファイルの
// 先頭に同梱している）。
// game.infinity.broken は廃止し、game.breakInfinity.unlocked を
// 唯一の判定フラグとする。
//
// 解放ボタンについて:
//   Break Infinityタブ自体は解放されるまで非表示になるため、
//   解放ボタンはタブの中ではなく、常にアクセスできる「Infinity」
//   タブ側（#break-infinity-unlock-section）に置いている。
//   解放後の内容（ポイント・アップグレード等）は専用タブ側で
//   表示する。
//
// 依存関係:
//   - グローバル変数 `game`（script.js側で定義）を参照する。
//   - このファイルは script.js より先に読み込むこと
//     （script.js の getInitialState() がこのファイルの
//     getDefaultBreakInfinityState() を呼び出すため）。
// =========================================================

// --- 解放条件 ---
const BREAK_INFINITY_UNLOCK_IP = 1e50; // 所持IPがこの値以上でBreak Infinityを解放できる

// --- Break Infinity アップグレード定義（配列で管理・追加しやすい構造） ---
const BREAK_INFINITY_UPGRADES = [
  {
    id: 'breakPower',
    title: 'Break Power',
    desc: '全生産倍率を強化する（レベル制、購入毎にコストが上昇）',
    baseCost: 1,
    costMult: 3,
    effect: (level) => 1 + level * 0.5,      // 現在の効果（生産倍率）
...
function getDefaultBreakInfinityState() {
  return {
    ...
    points: 0,        // Break Infinity通貨
    upgrades: {}       // 購入済みアップグレードのレベル（id: level）
  };
}

// 既存セーブとの互換性維持: 不足データを自動補完する
function ensureBreakInfinityState() {
  if (!game.breakInfinity) {
    game.breakInfinity = getDefaultBreakInfinityState();
    return;
  }
  const def = getDefaultBreakInfinityState();
  if (typeof game.breakInfinity.unlocked !== 'boolean') game.breakInfinity.unlocked = def.unlocked;
  if (typeof game.breakInfinity.points !== 'number' || isNaN(game.breakInfinity.points)) game.breakInfinity.points = def.points;
  if (!game.breakInfinity.upgrades || typeof game.breakInfinity.upgrades !== 'object') game.breakInfinity.upgrades = def.upgrades;
}

// Break Infinityが解放・有効化されているか（true の場合、粒子数の上限を撤廃する）
function isBreakInfinityActive() {
  return !!(game.breakInfinity && game.breakInfinity.unlocked);
}

// --- アップグレードのレベル・コスト ---
function getBreakInfinityUpgradeLevel(id) {
  ensureBreakInfinityState();
  return game.breakInfinity.upgrades[id] || 0;
}

function getBreakInfinityUpgradeCost(up) {
  const level = getBreakInfinityUpgradeLevel(up.id);
  return up.baseCost * Math.pow(up.costMult, level);
}

// Break Infinityによる全体倍率（今後アップグレードが増えても自動で合算される）
function getBreakInfinityMultiplier() {
  ensureBreakInfinityState();
  let mult = 1;
  BREAK_INFINITY_UPGRADES.forEach(up => {
    const level = getBreakInfinityUpgradeLevel(up.id);
    if (level > 0) mult *= up.effect(level);
  });
  return mult;
}
...
function tryUnlockBreakInfinity() {
  ...
  if (currentIP < BREAK_INFINITY_UNLOCK_IP) return;

  game.infinity.ip -= BREAK_INFINITY_UNLOCK_IP;
  game.breakInfinity.unlocked = true;
  if (typeof showModal === 'function') {
    showModal({
      title: '上限突破',
      body: '粒子の上限が撤廃されました。<br>Break Infinityが解放されました。',
  ...
  updateBreakInfinitySectionVisibility();
  if (typeof updateChallengeSectionVisibility === 'function') updateChallengeSectionVisibility();
}

// Infinityタブに表示する解放セクション（Break Infinityタブ自体は解放されるまで
// アクセスできないため、解放操作はここ＝常にアクセスできるInfinityタブから行う）
function updateBreakInfinityUnlockSection() {
  const section = document.getElementById('break-infinity-unlock-section');
  if (!section) return;
  ensureBreakInfinityState();
  ...
  if (game.breakInfinity.unlocked) {
    ...
    const currentIP = (game.infinity && game.infinity.ip) ? game.infinity.ip : 0;
    btn.classList.toggle('disabled', currentIP < BREAK_INFINITY_UNLOCK_IP);
  }
}

// --- アップグレード購入 ---
function buyBreakInfinityUpgrade(id) {
  ensureBreakInfinityState();
  const up = BREAK_INFINITY_UPGRADES.find(u => u.id === id);
  if (!up) return;
  const cost = getBreakInfinityUpgradeCost(up);
  if (game.breakInfinity.points < cost) return;

  game.breakInfinity.points -= cost;
  game.breakInfinity.upgrades[id] = (game.breakInfinity.upgrades[id] || 0) + 1;
...
// Big Crunch毎にBreak Infinityポイントを付与する（Infinity解放後のIPと同じ考え方）
function grantBreakInfinityPointsOnCrunch() {
  ensureBreakInfinityState();
  if (!game.breakInfinity.unlocked) return;
  game.breakInfinity.points = (game.breakInfinity.points || 0) + 1;
}

// --- タブの表示/非表示（解放されるまで自動で非表示） ---
function updateBreakInfinitySectionVisibility() {
  const tabBtn = document.getElementById('tab-btn-break-infinity');
  if (!tabBtn) return;
  ensureBreakInfinityState();
  tabBtn.style.display = game.breakInfinity.unlocked ? 'block' : 'none';
}

// --- Break Infinity画面の描画 ---
function updateBreakInfinityTab() {
  const container = document.getElementById('break-infinity-content');
  if (!container) return;
  ensureBreakInfinityState();
  ...
  if (!game.breakInfinity.unlocked) {
    ...
    container.innerHTML = `
      <p style="color:#aaa; font-size:0.85rem; margin-bottom:15px;">
        粒子の上限（1.79e308）を撤廃し、さらに先へ進めるようになります。
      </p>
      <div class="stat-row">
        <span class="stat-label">所持 IP</span>
        <span class="stat-val" id="bi-current-ip">0</span>
      </div>
      <button id="bi-unlock-btn" class="ui-btn" style="border-color:#ff0055; color:#ff0055; margin-top:15px;" onclick="tryUnlockBreakInfinity()">
        上限を解除 (必要: 1.00e50 IP)
      </button>
    `;
    const ipEl = document.getElementById('bi-current-ip');
    if (ipEl) ipEl.textContent = format(currentIP);
    const btn = document.getElementById('bi-unlock-btn');
    if (btn) {
      if (currentIP >= BREAK_INFINITY_UNLOCK_IP) btn.classList.remove('disabled');
      else btn.classList.add('disabled');
    }
    return;
  }

  // 解放後: ポイント表示とアップグレード一覧
  if (!container.dataset.unlockedInitialized) {
    container.dataset.unlockedInitialized = '1';
    container.innerHTML = `
      <p>所持ポイント: <span id="bi-points-display" style="color:#fff; font-weight:bold;">0</span></p>
      <div id="bi-upgrades-container" class="inf-upgrade-grid" style="margin-top:15px;"></div>
    `;
    const upgContainer = document.getElementById('bi-upgrades-container');
    BREAK_INFINITY_UPGRADES.forEach(up => {
      const btn = document.createElement('div');
      btn.className = 'inf-upgrade-btn';
      btn.id = `bi-upgrade-${up.id}`;
      btn.innerHTML = `
        <div style="width:100%">
          <div class="inf-desc">${up.title}: ${up.desc}</div>
          <div class="inf-effect-val" style="font-size:0.8em; color:#ff0055;"></div>
        </div>
        <div class="inf-cost"></div>
      `;
      btn.addEventListener('click', () => buyBreakInfinityUpgrade(up.id));
      upgContainer.appendChild(btn);
    });
  }

  const ptsEl = document.getElementById('bi-points-display');
  if (ptsEl) ptsEl.textContent = format(game.breakInfinity.points);

  BREAK_INFINITY_UPGRADES.forEach(up => {
    const btn = document.getElementById(`bi-upgrade-${up.id}`);
    if (!btn) return;
    const level = getBreakInfinityUpgradeLevel(up.id);
    const cost = getBreakInfinityUpgradeCost(up);
    const currentEffect = up.effect(level);
    const nextEffect = up.effect(level + 1);

    btn.classList.toggle('disabled', game.breakInfinity.points < cost);
    const effectEl = btn.querySelector('.inf-effect-val');
    if (effectEl) effectEl.innerHTML = `Lv.${level}　現在：${up.formatEffect(currentEffect)}　次：${up.formatEffect(nextEffect)}`;
    const costEl = btn.querySelector('.inf-cost');
    if (costEl) costEl.textContent = 'コスト: ' + format(cost) + ' pt';
  });
}

// --- 初期化処理 ---
function initBreakInfinity() {
  ensureBreakInfinityState();
  updateBreakInfinitySectionVisibility();
  updateBreakInfinityUnlockSection();
  updateBreakInfinityTab();
